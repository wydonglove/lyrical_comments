...
# model_path ='d:/data/wordVec/sgns.target.word-word.dynwin5.thr10.neg5.dim300.iter5'
model_path ='/home/yangyanqi/models/sgns.target.word-word.dynwin5.thr10.neg5.dim300.iter5'
...
model = textCNN_Kim(input_width=1,input_dim=input_dim,class_num=2,kernel_num=100,kernel_sizes=[2,3,4,5])
model =model.cuda()
...
train_id, test_id = corpus_id[0:int(len(corpus_id) * 0.8)], corpus_id[int(len(corpus_id) * 0.8):]
if sess == 'train':
    model.train()
    ...
    for epoch in range(1, epoch+1):
        ...
        for batches in train_batches_id:
            ...
            # if batch_num==i:
        save(model, save_dir, 'model',epoch)
        steps = 0
            # sess='eval'
if sess == 'eval':
    batch_id_tmp = []
    ...
    print('eval')
    model.load_state_dict(torch.load('weights/snapshot.pt'))
    model.eval()
    corrects, avg_loss = 0, 0
    tp,fn,tn,fp = 0,0,0,0
